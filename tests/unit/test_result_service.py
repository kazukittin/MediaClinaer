...
def test_result_service_returns_detection_group_details(tmp_path):
    ...
    DetectionService(database, logger, config).detect_quality_issues(scan_result.session_id)

    details = ResultService(database).list_detection_group_details(scan_result.session_id)

    assert details
    assert any(detail.summary.group_type == "blurry_image" for detail in details)
    assert any(item.path.endswith("blurry.jpg") for detail in details for item in detail.items)
...
def test_result_service_updates_detection_item_selection(tmp_path):
    media_dir = tmp_path / "media"
    media_dir.mkdir()
    write_test_image(media_dir / "sharp.jpg")
    write_blurry_test_image(media_dir / "blurry.jpg")
