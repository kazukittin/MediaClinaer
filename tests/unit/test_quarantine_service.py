...
def test_quarantine_service_moves_selected_blurry_candidates(tmp_path):
    media_dir = tmp_path / "media"
    media_dir.mkdir()
    keep_file = media_dir / "sharp.jpg"
    blurry_file = media_dir / "blurry.jpg"
    write_test_image(keep_file)
    write_blurry_test_image(blurry_file)
    ...
    DetectionService(database, logger).detect_quality_issues(scan_result.session_id)
    ...
    assert result.items[0].original_path == str(blurry_file)
    assert result.items[0].quarantined_path.endswith("blurry.jpg")
    assert keep_file.exists()
    assert not blurry_file.exists()
    ...
    assert rows[0][1] == str(blurry_file)
    assert rows[0][2].endswith("blurry.jpg")


def test_quarantine_service_deletes_readonly_blurry_after_copy(tmp_path):
    media_dir = tmp_path / "media"
    media_dir.mkdir()
    keep_file = media_dir / "sharp.jpg"
    blurry_file = media_dir / "blurry.jpg"
    write_test_image(keep_file)
    write_blurry_test_image(blurry_file)
    blurry_file.chmod(stat.S_IREAD)
